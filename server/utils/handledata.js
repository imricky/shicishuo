...
// 用来导入唐诗和宋词，放到一个数据库里，并加上标签, 这段代码是做网站之前处理的
// 诗词存放仓库 github：https://github.com/chinese-poetry/chinese-poetry

const fsPromises = require('fs').promises;
const fs = require('fs');
const path = require('path');
const filePath = path.resolve(`${__dirname}/tangandsong`);
...
const Poet = mongoose.model('allPoets', poetSchema);
...
async function readDirFile() {
  ...
  for (const item of dir) {
    ...
    const res = await fsPromises.readFile(`./tangandsong/${item}`);
    console.log(item);
...
function jtgo(cc) {
  ...
  for (let i = 0; i < cc.length; i++) {
    if (charft().indexOf(cc.charAt(i)) != -1) str += charjt().charAt(charft().indexOf(cc.charAt(i)));
    else if (charhx().indexOf(cc.charAt(i)) != -1) str += charjt().charAt(charhx().indexOf(cc.charAt(i)));
    else str += cc.charAt(i);
  }
  return str;
}

// 传入一个对象，将对象的每一个title(string)和paragraphs(array)都繁体转简体，再返回去

function changeObj(obj) {
  const final = JSON.parse(JSON.stringify(obj));
  final.author = jtgo(obj.author);
  final.title = jtgo(obj.title);
  for (let i = 0; i < final.paragraphs.length; i++) {
    final.paragraphs[i] = jtgo(final.paragraphs[i]);
  }
  if (final.tags) {
    for (let i = 0; i < final.tags.length; i++) {
      final.tags[i] = jtgo(final.tags[i]);
    }
  }
  return final;
}
